...
<script>
    omnivaltdelivery_controller="{$link->getModuleLink('omnivaltshipping', 'ajax')}";
</script>
<div id="omnivalt_parcel_terminal_carrier_details" style="display: block; margin-top: 10px;">
    <select class="select2" name="omnivalt_parcel_terminal" style="max-width:400px;">{$parcel_terminals nofilter}</select>
...
{if isset($omniva_api_key) and $omniva_api_key != false }
    ...
    <script type="text/javascript">
        var locations = {$terminals_list nofilter};
  ...
  var markerSymbol = {
   ...
   height: "30px"
    ...
    searchWidget.on("select-result", function(event) {
      ...
      <button type="button" id="show-omniva-map" class="btn btn-basic btn-sm omniva-btn " data-toggle="tooltip" data-placement="top" title="{l s='Teminalų paieška žemelapyje'}"><i id="show-omniva-map" class="fa fa-map-marker-alt fa-lg" aria-hidden="true"></i></button>
{/if}
</div>
